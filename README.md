...
flowchart LR
    ...
    class I,I1,I2,J output;
```

The architecture follows the published methodology: preprocessing and temporal representation are followed by self-attention Transformer modeling, sequential AOA→RFO optimization, and final classification.

## Dataset

PULSE contains **14,010 activity sequences**, represented using **5 sequential time steps and 11 final feature dimensions** after preprocessing. The features encode user behavior, access context, activity, resource usage, temporal information, and other operational signals.

The paper describes binary labels for benign versus insider activity and four multiclass threat categories based on logon/logoff frequency, file access activity, removable-media usage, and process execution. Class imbalance is addressed through oversampling of minority classes and controlled undersampling of majority classes.
...
The published workflow includes handling incomplete or corrupted records, timestamp normalization, user-identity mapping, categorical encoding, and Min-Max normalization. The processed dataset is represented as a tensor with shape **[14010, 5, 11]**.

### 2. Self-attention Transformer

The model uses input embedding, positional encoding, multi-head self-attention, and feed-forward processing to learn temporal and contextual relationships across user activity sequences. Query/key/value attention dynamically weights relationships between elements of the sequence.
...
The objective is to balance global exploration and local exploitation during Transformer tuning.

### 4. Classification

The final model supports both binary insider-threat classification and multiclass classification across four scenario categories. The reported evaluation includes accuracy, precision, recall, F1-score, false-positive rate, false-negative rate, and Matthews correlation coefficient.
...
The paper reports strong class-level F1 scores in both binary and multiclass settings and evaluates false-positive and false-negative behavior across the four multiclass scenarios.

### Comparison with reported baselines

The paper's comparative analysis reports the proposed approach at **97.67% accuracy**, compared with 95.2% for Random Forest, 90.6% for LSTM, 94.7% for RNN, 96.61% for an attention-based method, and 96.3% for ANN in the cited comparison table.
...
The published study notes that the current evaluation is based on static datasets rather than streaming operational data. The paper identifies streaming-data extension and adversarial robustness as directions for future work.

## Publication

**Scenario Driven Insider Threat Detection Using Dual Modelling Architecture**

Authors: P. Lavanya, Pullela Vaishnavi, **Pullela Giridhar**, H. Anila Glory, V. S. Shankar Sriram.
